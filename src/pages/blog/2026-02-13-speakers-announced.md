---
layout: "../../layouts/BlogLayout.astro"
title: "RustWeek Speakers Announced!"
pubDate: 2026-02-13
---

We’ve been reading, rating, discussing, and re-rating your submissions for the past few weeks, but the wait is finally over. We’re really excited to announce the speakers that will be taking the stage at Rustweek 2026! 

In case you haven’t yet, now is a great time to grab your tickets at https://event.onliveevent.nl/rustweek-2026. But without further ado, here are the speakers that have been selected:

- One Language, Two IDE Engines - Vlad Beskrovny & Lukas Wirth
- Obsessive Optimization with String Interning - Arya Dradjica
- Shaping Tomorrow’s Software Engineers - Mordecai emmanuel Etukudo
- Out of tree access to compiler state - Alona Enraght-Moony
- Writing a top-10 chess engine in Rust - Kora & Cosmo Bobak
- Field report on scripting Rust - Gergely Nagy
- Precise, consistent, and reliable code coverage - Jynn Nelson
- How we replace common C(++) libraries with Rust at scale - Bastian Kersting
- Writing shaders for graphics in plain rust - Sebastian Sydow
- When uom isn't enough: Handling units of measurement in production CAD applications - Rose Peck
- Open Code Review: Trust Fosters Responsibility - Alice Cecile
- Creativity loves constraint (solvers) - Adam Chalmers
- The trials of qualifying Rust - Aïssata Maïga
- Programming Your Own DSL in Rust – Experiences from Developing µcad - Michael Winkelmann
- When Iterators Aren't Zero Cost - Xavier Denis
- Expanding Processing's Reach with Rust + Bevy - Moon Davé
- Debunking Rust Wasm Performance Myths: Why We Moved Core Business Logic to Rust at Canva - Taj Pereira & Andrew Jakubowicz 
- Learning Rust as First Programming Language - Karin Lammers
- Stabilizing decade-old features - Folkert de Vries
- Tracking down undefined behaviour in Servo - Josh Bowman-Matthews
- Compilers are just programs: Fixing a bug in the Rust compiler - Boxy N/A
- One year of facet: reflections on reflection - Amos Wenger
- Making sound - dragging a ten year old crate into 2026 - Yara Kleingeld
- Typst: Designing for Incrementality - Laurenz Mädje
- When is never? - Waffle -
- Common Pitfalls of Rewriting Things In Rust - Cliff L. Biffle
- Blitz: building a modular web engine - Nico Burns
- Racing with Rust: building telemetry tools for racing simulators. - Harry van Haaren
- Knock, knock. Race condition! Who's there? - A Refactoring Story - Stefan Baumgartner
- Programming Languages as a Community of Belonging - Hazel Weakly

They will be joining our invited speakers:
- Æva Black
- Alice Rhyl
- Andreea Costea
- Andrew ‘BurntSushi’ Gallant
- Greg Kroah-Hartman
- Jeremy Lempereur
- Lori Lorusso
- Mark Janssen
- Pete LeVasseur
- Taylor Cramer

More details about all these talks will be published soon!
